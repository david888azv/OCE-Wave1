"""Run SIESTA SZP on the 50 active-learning iter-2 (non-strain) picks."""
...
def main(workers: int = 6):
    ...
    selection = json.loads((sd / "active_learning_iter2_selection.json").read_text())
    ...
    print(f"AL iter-2 batch: {len(selection)}  to run: {len(queue)}  "
    ...
    print(f"\nAL iter 2 done: {n_ok} converged / {n_fail} failed")
