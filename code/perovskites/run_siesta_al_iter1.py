"""Run SIESTA SZP on the 50 active-learning-selected perovskites.

Resumes from siesta_szp_results.json (will append, not overwrite).  Output:
extends data/perovskites/siesta_szp_results.json so the n=143 set is
immediately available for refit.
"""
...
def main(workers: int = 6):
    ...
    selection = json.loads((sd / "active_learning_iter1_selection.json").read_text())
    ...
    print(f"AL batch: {len(selection)}  already done: {len(selection)-len(queue)}  "
          f"to run: {len(queue)}  workers: {workers}×2 OMP")

    done = list(prior)
    n_ok_new = 0; n_fail_new = 0
    t0 = time.time()
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for fut in as_completed(futs):
            ...
            if r.get("converged"): n_ok_new += 1
            else: n_fail_new += 1
            elapsed = time.time() - t0
            n_so_far = n_ok_new + n_fail_new
    ...
    print(f"\nAL iter 1 done: {n_ok_new} converged / {n_fail_new} failed")
